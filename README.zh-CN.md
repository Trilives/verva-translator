...
- 可保存并快速切换多套配置；API 密钥加密保存；每套配置可独立设置思考等级并开启长会话。
...
4. 填写基础 URL，并选择或输入模型名。公网端点必须使用 HTTPS；本机和私有局域网地址可使用 HTTP，且无需 API 密钥。
5. 按需设置思考等级或开启长会话。
6. 所有更改都会自动保存；点击**测试连接**可确认接口是否可用。
7. 返回工作区，在顶部选择该配置。
...
- 公网模型必须使用 HTTPS；本机回环和私有局域网端点允许 HTTP。
